...
FROM Teams)
SELECT T.team_id, 300*CAST(R.matches_won AS FLOAT)/R.matches_played AS Rating, C.country_name FROM Teams T INNER JOIN results R ON T.team_id = R.team_id INNER JOIN Countries C ON T.country_code = C.country_code 
...
(SELECT R.roll_no, 
(SELECT 50*(CAST(SUM(S.bat_runs) AS FLOAT)/(1+SUM(S.bat_balls)) + 20*CAST(SUM(S.bat_runs) AS FLOAT)/(1+COUNT(S.bat_was_out))) FROM Player_Match_Stats S INNER JOIN Matches M ON S.match_id = M.match_id AND YEAR(CAST(M.date_time AS DATE)) = @year WHERE S.roll_no = R.roll_no GROUP BY S.roll_no) AS BattingStats,
(SELECT 50*(1.5 - (CAST(SUM(S.ball_runs_conceded) AS FLOAT)/(1+SUM(S.balls_bowled))) + 20*CAST(SUM(S.ball_wickets) AS FLOAT)/(1+COUNT(S.balls_bowled))) FROM Player_Match_Stats S INNER JOIN Matches M ON S.match_id = M.match_id AND YEAR(CAST(M.date_time AS DATE)) = @year WHERE S.roll_no = R.roll_no GROUP BY S.roll_no) AS BowlingStats
FROM rolls R)
